...
describe('filterUtils', () => {
  ...
  describe('getUsernameFromFilter', () => {
    ...
    test('should extract username from OR compound filter: (|(uid=user1)(uid=user2))', () => {
    ...
    test('should extract from NOT filter: (!(uid=blocked))', () => {
  ...
  describe('parseGroupFilter', () => {
    ...
    test('should handle OR filters: (|(cn=admins)(cn=users))', () => {
      const result = parseGroupFilter('(|(cn=admins)(cn=users))');
      
      expect(result.cn).toBe('admins'); // Returns first match
    });
    
    test('should handle filters with whitespace: (cn=developers)', () => {
